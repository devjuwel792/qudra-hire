      ...
      <div className="w-full max-w-[400px] bg-surface-card border border-surface rounded-3xl p-6 sm:p-10 shadow-2xl">
        ...
        <div className="flex bg-surface-deep border border-surface p-1 rounded-2xl mb-6">
          <button
            ...
            className={`flex-1 py-2.5 text-sm font-medium rounded-xl transition-all ${accountType === "candidate" ? "bg-[#4BC957] text-[#080C14]" : "text-on-surface-muted hover:text-on-surface"}`}
          >
            Candidate
          </button>
          <button
            onClick={() => setAccountType("company")}
            className={`flex-1 py-2.5 text-sm font-medium rounded-xl transition-all ${accountType === "company" ? "bg-[#4BC957] text-[#080C14]" : "text-on-surface-muted hover:text-on-surface"}`}
          >
            Company
          </button>
        </div>
        ...
        <form className="space-y-4" onSubmit={(e) => { e.preventDefault(); router.push(accountType === "candidate" ? "/candidate" : "/company"); }}>
          <div>
            ...
            <input type="email" placeholder="you@work.com" className="w-full bg-surface-deep border border-surface rounded-xl px-4 py-3 text-sm text-on-surface placeholder:text-on-surface-subtle focus:outline-none focus:border-[#4BC957]/50 transition-colors" />
          </div>
          <div>
            <label className="block text-xs font-medium text-on-surface mb-1.5">Password</label>
            <input type="password" placeholder="••••••••" className="w-full bg-surface-deep border border-surface rounded-xl px-4 py-3 text-sm text-on-surface placeholder:text-on-surface-subtle focus:outline-none focus:border-[#4BC957]/50 transition-colors" />
          </div>
