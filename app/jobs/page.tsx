    ...
    <div className="min-h-screen flex flex-col bg-[#080C14] text-white">
      ...
      <section className="border-b border-white/5 bg-[#0A0F1D] py-12">
        <div className="max-w-7xl mx-auto px-4 sm:px-8">
          ...
          <p className="text-slate-400 text-lg mb-8 animate-[fadeInUp_0.7s_0.15s_ease_forwards] opacity-0">
      ...
      <section className="flex-1 max-w-7xl mx-auto px-4 sm:px-8 py-10 w-full flex flex-col md:flex-row gap-8">
        ...
        <Animate className="animate-from-left w-full md:w-64 flex-shrink-0 space-y-6">
          ...
          {[
            ...
            <div key={title} className="bg-[#0A0F1D] border border-white/5 rounded-2xl p-5">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
            {jobs.map((job, idx) => (
              <Animate key={idx} className="animate-on-scroll" delay={delays[idx % 6]}>
                <div onClick={()=>router.push("/candidate/jobs/detail")} className="bg-[#0A0F1D] border border-white/5 rounded-2xl p-6 hover:border-white/10 transition-colors cursor-pointer group h-full">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {job.tags.map((tag: string) => (
                      <span key={tag} className="bg-[#162032] border border-white/5 text-slate-300 text-[11px] font-medium px-2.5 py-1 rounded-md">{tag}</span>
                    ))}
                  </div>
                </div>
              </Animate>
