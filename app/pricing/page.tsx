    ...
    <div className={`relative flex flex-col rounded-2xl p-6 transition-all duration-300 ${
      ...
      {plan.popular && (
        <div className="absolute -top-3.5 left-1/2 -translate-x-1/2">
          <span className="bg-[#00D07C] text-[#080C14] text-xs font-bold px-3 py-1 rounded-full shadow-lg">Most popular</span>
        </div>
      )}
      ...
      <ul className="space-y-2.5 flex-1">
        {plan.features.map(f => (
          <li key={f} className="flex items-start gap-2.5 text-sm text-slate-400">
            <span className={`mt-0.5 flex-shrink-0 h-4 w-4 rounded-full flex items-center justify-center ${plan.popular ? "bg-[#00D07C]/15 text-[#00D07C]" : "bg-slate-700/50 text-slate-400"}`}>
            ...
            {(["candidates", "companies"] as const).map(t => (
              <button key={t} onClick={() => setTab(t)}
                className={`px-5 py-2.5 rounded-lg text-sm font-semibold transition-all ${
                  tab === t ? "bg-[#00D07C] text-[#080C14] shadow-md" : "text-slate-400 hover:text-white"
                }`}>
      ...
      <section className="max-w-5xl mx-auto px-4 sm:px-8 pb-28 w-full">
        ...
        <Animate className="animate-on-scroll" delay="anim-delay-300">
          <p className="text-center text-xs text-slate-100 mt-10">
            ...
            <Link href="/contact" className="text-green-500 hover:text-[#00D07C] transition-colors underline underline-offset-2">Contact us</Link>{" "}
